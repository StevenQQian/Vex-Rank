...
Date: 2026-09-15. Status: implemented in the website calculation paths and scenario-tested; not deployed or historically calibrated.
...
The reference source is `algorithm/vcr-event.mjs`. The website implementation is `website/lib/vcr3.mjs`, used by the live ranking API, team rating history, and historical ranking builder. Tests are in `algorithm/vcr-event.test.mjs` and `website/tests/vcr3.test.mjs`. Run from the repository root:

```sh
node --test algorithm/vcr-event.test.mjs
node --test website/tests/vcr3.test.mjs
```
...
The website now calls the VCR 3.0 settlement core from `website/app/api/rankings/route.ts`, `website/app/api/teams/[number]/route.ts`, and `website/scripts/build-historical-ranking.mjs`. API responses include `modelVersion`, while event history includes the component ledger and achievement correction. The current adapter estimates elimination expectation and title probability from the frozen event field because a full schedule/bracket simulator is not yet available. Contribution and autonomous residuals remain neutral when the official feed lacks enough attribution data. Historical calibration and full-field replay are still required before removing the candidate label or deploying the model as authoritative.
