...
function RankingsView({ teams: rows, meta, openTeam, savedState, setSavedState }: { teams: any[]; meta:any; openTeam:(t:any)=>void;savedState:any;setSavedState:(state:any)=>void }) {
  ...
  return <section className="mx-auto max-w-[1440px] px-5 py-10 lg:px-8"><PageHead eyebrow="Live VEX Competitive Rating" title="World team ranking" copy="Compare current standings with past seasons, then narrow by school level, country and event region." />
    ...
    <p className="mt-3 text-xs text-white/40">{isFullArchive?'VCR 3.0 · completed-season archive.':`Live official-data sample${activeMeta?.eventsProcessed?` · ${activeMeta.eventsProcessed} events`:''}. The complete season archive is not available yet.`}</p>
    <div className="mt-6 overflow-hidden border border-white/10 bg-[#101319]"><Table><TableHeader><TableRow className="border-white/10 hover:bg-transparent"><TableHead className="w-20 px-5 text-[10px] font-semibold uppercase tracking-wider text-white/35">Rank</TableHead><TableHead className="text-[10px] font-semibold uppercase tracking-wider text-white/35">Team</TableHead><TableHead className="text-[10px] font-semibold uppercase tracking-wider text-white/35">Region</TableHead><TableHead className="text-[10px] font-semibold uppercase tracking-wider text-white/35">Record</TableHead><TableHead className="text-[10px] font-semibold uppercase tracking-wider text-white/35">Matches</TableHead><TableHead className="px-5 text-right text-[10px] font-semibold uppercase tracking-wider text-white/35">VCR</TableHead></TableRow></TableHeader><TableBody>{visibleRows.map((t:any,index:number) => <TableRow key={t.number} tabIndex={0} role="link" onKeyDown={e=>{if(e.key==='Enter')openTeam({...t,season,seasonId:RANKING_SEASONS[season].id})}} onClick={() => openTeam({...t,season,seasonId:RANKING_SEASONS[season].id})} className="cursor-pointer border-white/[.07] hover:bg-white/[.04]"><TableCell className="px-5"><span className={`text-lg font-semibold ${index < 3 ? 'text-[#ed2b3a]' : 'text-white/40'}`}>#{index+1}</span></TableCell><TableCell><b>{t.number}</b><small className="ml-3 text-white/35">{t.name}</small></TableCell><TableCell className="text-white/45">{t.region}</TableCell><TableCell className="font-mono text-white/65">{t.record}</TableCell><TableCell className="text-white/45">{t.matches}</TableCell><TableCell className="px-5 text-right font-mono text-lg font-semibold">{t.rating}<small className="ml-2 text-xs text-white/25">±{t.confidence}</small></TableCell></TableRow>)}</TableBody></Table></div>
    ...
    <p className="mt-4 text-xs text-white/30">Provisional VCR uses the specification’s opponent-adjusted match model and conservative uncertainty adjustment. Elimination results contribute to VCR 3.0. Contribution and autonomous components remain neutral where data is unavailable.</p>
  </section>;
}
    ...
    <div className="p-5">
      <div className="mb-5 flex flex-col gap-2 rounded-r-lg border-l-2 border-[#ed2b3a] bg-white/[.025] p-4 sm:flex-row sm:items-center">
        <div className="min-w-0">
          <p className="text-[11px] text-white/35">{new Date(detail.eventDate).toLocaleDateString(undefined,{year:'numeric',month:'long',day:'numeric'})}</p>
...
function TeamView({ team, goBack, backLabel, openEvent }: { team:any; goBack:()=>void;backLabel:string;openEvent:(event:any)=>void }) {
  ...
  return <section><div className="border-b border-white/10 bg-[#0b0d11]"><div className="mx-auto max-w-[1440px] px-5 pb-12 pt-8 lg:px-8"><button onClick={goBack} className="flex items-center gap-2 text-xs text-white/40 hover:text-white"><ArrowLeft className="h-4 w-4" /> Back to {backLabel}</button>
      <div className="team-hero-parallax">
      <div className="rise mt-10 flex flex-wrap items-baseline gap-x-3 gap-y-1 text-[11px] font-bold uppercase tracking-[.22em] text-white/35" style={{'--i':0} as React.CSSProperties}><span>V5RC team profile</span><span className="text-white/15">/</span><span>{history?(profile.active?'Active':'Inactive'):(team.rank?`World #${team.rank}`:'Loading')}</span>{profile.region&&<><span className="text-white/15">/</span><span>{profile.region}</span></>}</div>
...
function PageHead({ eyebrow, title, copy }: {eyebrow:string;title:string;copy:string}) { return <div className="max-w-3xl border-b border-white/[.08] pb-6"><p className="text-[11px] italic text-white/30">{eyebrow}</p><h1 className="font-display mt-1.5 text-[2.6rem] leading-[1.05] sm:text-[3.4rem]">{title}</h1><p className="mt-4 max-w-xl text-sm leading-relaxed text-white/45">{copy}</p></div>; }
function SectionTitle({ eyebrow, title }: {eyebrow:string;title:string}) { return <div className="flex items-baseline justify-between gap-4 border-b border-white/[.08] px-5 pb-3 pt-5"><h2 className="font-display text-[1.35rem] leading-tight">{title}</h2><p className="shrink-0 text-[11px] italic text-white/30">{eyebrow}</p></div>; }
function Metric({ label, value, detail }: {label:string;value:string;detail:string}) { return <div className="border-t border-white/15 pt-3"><small className="text-[11px] text-white/35">{label}</small><p className="mt-1.5 font-display text-[1.75rem] leading-none">{value}</p><small className="mt-1.5 block text-xs text-white/30">{detail}</small></div>; }
function StatFeature({ icon, title, copy }: {icon:React.ReactNode;title:string;copy:string}) { return <article className="border-t border-white/15 pt-5"><span className="text-white/25 [&_svg]:h-4 [&_svg]:w-4">{icon}</span><h3 className="font-display mt-3 text-lg">{title}</h3><p className="mt-2 text-sm leading-relaxed text-white/40">{copy}</p></article>; }
function Empty({ text }: {text:string}) { return <p className="border-t border-white/[.08] px-1 py-10 text-sm italic leading-relaxed text-white/35">{text}</p>; }
function FilterInput({ value, onChange, placeholder, icon }: {value:string;onChange:(value:string)=>void;placeholder:string;icon?:React.ReactNode}) { return <label className="relative">{icon && <span className="absolute left-0 top-2.5 text-white/30 [&_svg]:h-4 [&_svg]:w-4">{icon}</span>}<input value={value} onChange={event=>onChange(event.target.value)} placeholder={placeholder} aria-label={placeholder} className={`h-9 w-full border-0 border-b border-white/15 bg-transparent pr-3 text-sm outline-none placeholder:text-white/30 focus:border-white/40 ${icon ? 'pl-6' : 'pl-0'}`} /></label>; }
