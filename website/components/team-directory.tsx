...
export default function TeamDirectory({search,setSearch,country,setCountry,region,setRegion,grade,setGrade,openTeam}:any){
  ...
  return <section className="mx-auto max-w-[1440px] px-5 py-10 lg:px-8">
    ...
    <div className="mt-8 grid gap-3 border border-white/10 bg-[#101319] p-4 sm:grid-cols-3 xl:grid-cols-[2fr_1fr_1fr_1fr]">
      <label className="relative sm:col-span-3 xl:col-span-1"><Search className="absolute left-3 top-3 h-4 w-4 text-white/35"/><input aria-label="Search team directory" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Try 8829C or 55288A" className="h-10 w-full border border-white/10 bg-black/20 pl-9 pr-3 text-sm outline-none focus:border-white/30"/></label>
    ...
    {error?<div role="alert" className="mt-5 border border-rose-400/30 p-5 text-sm text-rose-300">{error}<button onClick={()=>setRetry(n=>n+1)} className="ml-4 border border-white/20 px-3 py-2 text-white">Retry</button></div>:!directory?<p role="status" className="mt-5 text-sm text-white/50">Loading the team directory…</p>:<>
      ...
      <p className="mt-2 text-xs text-white/40">School level and location reflect each team’s latest official profile, including inactive teams.</p>
      <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{results.slice(0,limit).map((t:Team)=><button key={t.id} onClick={()=>openTeam({...t,region:[t.city,t.region,t.country].filter(v=>v&&v!=='Unassigned').join(', ')})} className="group border border-white/10 bg-[#101319] p-5 text-left hover:border-white/30">
        <div className="flex items-center justify-between"><h2 className="text-xl font-black group-hover:text-[#ed2b3a]">{t.number}</h2><ArrowUpRight className="h-4 w-4 text-white/35"/></div>
